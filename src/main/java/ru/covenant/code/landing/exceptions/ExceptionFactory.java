...
public final class ExceptionFactory {
    ...
    public static BusinessException persistenceError(String entity, String operation, Throwable cause) {
        return new PersistenceException(entity, operation, cause);
    }

    // Статические методы для быстрого создания

    public static BusinessException unauthorized(String message) {
        return new BusinessException("UNAUTHORIZED", "Не авторизован", message, HttpStatus.UNAUTHORIZED);
    }

    public static BusinessException forbidden(String message) {
        return new BusinessException("FORBIDDEN", "Доступ запрещен", message, HttpStatus.FORBIDDEN);
    }

    public static BusinessException badRequest(String message) {
        return new BusinessException("BAD_REQUEST", "Некорректный запрос", message, HttpStatus.BAD_REQUEST);
    }

    public static BusinessException internalError(String message) {
        return new BusinessException("INTERNAL_ERROR", "Ошибка сервера", message, HttpStatus.INTERNAL_SERVER_ERROR);
    }
